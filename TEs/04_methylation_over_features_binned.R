...
setwd("~/Dropbox/Research/Leicester_postdoc/Projects/IDLE/Ben_Developmental_BB/weighted_meth_new_annotation")
library(readr)
library(doBy)
library(ggplot2)
library(reshape2)
...
library(ggthemes)
## -------------------------------------------------------------------------
annotation1 <- read_delim("weighted_meth_annotation_by_stage_new_annotation.txt", 
                         "\t", escape_double = FALSE, trim_ws = TRUE)
annotation1 <-annotation1[,-5]

tes <- read_delim("~/Dropbox/Research/Leicester_postdoc/Projects/IDLE/Ben_Developmental_BB/TEs/weighted_meth_te/weighted_meth_annotation_by_stage_TEs.txt", 
                                                    delim = "\t", escape_double = FALSE, 
                                                    trim_ws = TRUE)
annotation <- rbind(annotation1, tes)

# removed feature column as running again with all genes, would need to add back in 
annotation <- annotation[,-c(3,4)]
...
ggplot(summary_all, aes(x=Feature, y=Weighted_Methylation, fill=Stage))+
  ...
  scale_x_discrete(breaks = c("promoter","five_prime_UTR","three_prime_UTR","exon","intron","intergenic","lnc_RNA", "TE"),
                   labels = c("Promoter","5' UTR","3' UTR","Exon","Intron","Intergenic","lnc RNA", "TE"),
                   limits =c("promoter","five_prime_UTR","three_prime_UTR","exon","intron","lnc_RNA","intergenic", "TE"))
...
#  ylab("Count")+
  ...
  scale_fill_manual(breaks = c("repro_brain","repro_ovaries","larvae","pupae", "male_brain","sperm"),
                    ...
                    values=c("#D55E00","#E69F00","#F0E442","#009E73","#0072B2","#CC79A7"),
                    limits =c("repro_brain","repro_ovaries","larvae","pupae", "male_brain","sperm"))+
  scale_x_discrete(breaks = c("promoter","five_prime_UTR","three_prime_UTR","gene","exon","intron","intergenic","lnc_RNA","TE"),
                   labels = c("Promoter","5' UTR","3' UTR","Gene","Exon","Intron","Intergenic","lnc RNA","TE"),
                   limits =c("promoter","five_prime_UTR","three_prime_UTR","gene","exon","intron","lnc_RNA","intergenic","TE"))
...
## -------------------------------------------------------------------------
# Take a quick look across TE types
