...
A_merged <- summaryBy(weightedMeth ~ feature + gene_id + start + end + number +
...
B_merged <- summaryBy(weightedMeth ~ feature + gene_id + start + end + number +
...
C_merged <- summaryBy(weightedMeth ~ feature + gene_id + start + end + number +
...
D_merged <- summaryBy(weightedMeth ~ feature + gene_id + start + end + number +
...
E_merged <- summaryBy(weightedMeth ~ feature + gene_id + start + end + number +
...
f_merged <- summaryBy(weightedMeth ~ feature + gene_id + start + end + number +
                        stage, data = f_all, FUN=mean)


all_data <- rbind(A_merged, B_merged, C_merged, D_merged, E_merged, f_merged)

all_data2 <- dcast(all_data, feature + gene_id + start + end + number  ~ stage, value.var = "weightedMeth.mean")

write.table(all_data2, file="weighted_meth_annotation_by_stage_new_annotation.txt", col.names = T,
